				...
				<div in:fly={{ y: 30, duration: 600 }}>
					...
					<div class="grid gap-8 lg:grid-cols-2 lg:items-center">
						...
						<div>
							<!-- Service Icon -->
							<div
								class="mb-6 flex h-20 w-20 items-center justify-center rounded-2xl bg-primary-500/10 transition-all duration-500 hover:scale-110 hover:bg-primary-500/20"
							>
				...
				<div class="space-y-12 lg:col-span-2">
					...
					{#if service.deliverables && service.deliverables.length > 0}
						<div in:fly={{ y: 30, delay: 600 }}>
							<h2 class="mb-6 text-3xl font-bold text-primary-400">Deliverables</h2>
							<div class="grid gap-3 sm:grid-cols-2">
								{#each service.deliverables as deliverable}
									<div
										class="flex items-start gap-3 rounded-xl border border-white/10 bg-gradient-to-br from-white/[0.03] to-transparent p-4 transition-all duration-300 hover:border-primary-500/50"
									>
										<div class="mt-0.5 text-primary-400">📦</div>
										<p class="text-secondary-300">{deliverable}</p>
									</div>
								{/each}
							</div>
						</div>
					...
					{#each relatedServices as relatedService, i}
						...
						>
							...
							<div class="relative z-10">
								<div
									class="mb-4 flex h-16 w-16 items-center justify-center rounded-2xl bg-primary-500/10 transition-all duration-500 group-hover:scale-110 group-hover:bg-primary-500/20"
								>
									{#if relatedService.icon.endsWith('.png') || relatedService.icon.endsWith('.svg') || relatedService.icon.endsWith('.jpg') || relatedService.icon.endsWith('.webp')}
										<img
											src={getServiceIconUrl(relatedService, relatedService.icon)}
											alt={relatedService.title}
											class="h-10 w-10 object-contain"
										/>
									{:else}
										<span class="text-4xl">{relatedService.icon}</span>
									{/if}
								</div>
